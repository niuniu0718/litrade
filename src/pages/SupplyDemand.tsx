...
const SupplyDemand: React.FC = () => {
  ...
  const demandBarOption = useMemo(() => ({
    ...
    xAxis: { type: 'category', data: demandSectors.map((d) => d.sector), axisLine: { lineStyle: { color: '#E8E8E8' } }, axisTick: { show: false }, axisLabel: { ...CHART_AXIS_LABEL, rotate: 20 } },
    yAxis: { type: 'value', name: '同比增长%', nameTextStyle: { color: '#8C8C8C', fontSize: 11 }, axisLabel: CHART_AXIS_LABEL, splitLine: CHART_SPLIT_LINE, axisLine: { show: false }, axisTick: { show: false } },
    series: [{ type: 'bar', data: demandSectors.map((d, i) => ({ value: d.growth, itemStyle: { color: d.growth >= 0 ? COLORS[i % COLORS.length] : '#FF4D4F' } })), barWidth: '40%' }],
  }), [demandSectors]);
      ...
      series: [
        ...
        },
        {
          ...
          data: balance.map((b) => ({
            value: b.demand,
            itemStyle: b.isForecast
              ? { color: 'rgba(250,173,20,0.3)', borderColor: '#FAAD14', borderWidth: 1, borderType: 'dashed' as const }
              : { color: '#FAAD14' },
          })),
          barWidth: '20%',
        },
    ...
    return {
      ...
      series: [{
        ...
        data: producers.map((p) => ({
          value: p.rate,
          itemStyle: {
            color: p.rate >= 85 ? '#00C86E' : p.rate >= 75 ? '#FAAD14' : '#FF4D4F',
  ...
  const elasticityOption = useMemo(() => {
    ...
    return {
      ...
      xAxis: { type: 'category', data: pricePoints.map((p) => (p / 10000) + '万'), axisLine: { lineStyle: { color: '#E8E8E8' } }, axisTick: { show: false }, axisLabel: { ...CHART_AXIS_LABEL, color: '#8C8C8C' } },
      yAxis: { type: 'value', name: '产能利用率%', min: 60, max: 105, nameTextStyle: { color: '#8C8C8C', fontSize: 11 }, axisLabel: CHART_AXIS_LABEL, splitLine: CHART_SPLIT_LINE, axisLine: { show: false }, axisTick: { show: false } },
      series: [{
        name: '产能利用率',
        type: 'line',
        data: supplyResponse,
        ...
        markLine: {
          ...
          data: [{ yAxis: 90, label: { formatter: '高利用率阈值', fontSize: 11, color: '#FF4D4F' } }],
        },
      }],
    };
  }, []);
  ...
  const projectColumns = [
    { title: '项目', dataIndex: 'name', key: 'name', width: 130, render: (v: string) => <span style={{ fontWeight: 600, color: '#1F1F1F' }}>{v}</span> },
  ...
  const supplyTab = (
    <div>
      {/* 汇总统计横条 */}
      <div style={{ ...CARD, background: '#FFFFFF', padding: '20px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 24, fontWeight: 700, color: '#1F1F1F' }}>{summary?.topCountries.length ?? '-'} <span style={{ fontSize: 12, fontWeight: 400, color: '#BFBFBF' }}>个</span></div>
        </div>
      </div>
      ...
      <Row gutter={GAP} style={{ marginTop: GAP }}>
        <Col xs={24} lg={12}>
          <div style={{ ...CARD, background: '#FFFFFF', ...CARD_BODY }}>
      ...
      {/* 供给弹性分析 */}
      <div style={{ ...CARD, background: '#FFFFFF', marginTop: GAP, ...CARD_BODY }}>
        <div style={SECTION}>供给弹性分析</div>
        <ReactECharts option={elasticityOption} style={{ height: 300 }} />
      </div>

      {/* 全球矿山项目表 */}
      <div style={{ ...CARD, background: '#FFFFFF', marginTop: GAP }}>
